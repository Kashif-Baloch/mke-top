...
async function scrapeMurfs() {
  ...
  try {
    ...
    const flavorData = await page.evaluate(() => {
      ...
      const flavorSections = document.querySelectorAll('div[class*="flavor"], section');
      ...
      headings.forEach(heading => {
        ...
        if (text.includes('Today') || text.includes('Mon') || text.includes('Tue') || 
          ...
          if (parent) {
            const flavorName = parent.textContent.match(/([A-Z][A-Za-z\s&']+)(?=\n|Vanilla|Chocolate|Cool mint|Caramel)/);
            const description = parent.textContent.split('\n').filter(line => 
    ...
    return [{
      ...
      flavors: flavorData.length > 0 ? flavorData : [
        {
          date: new Date().toISOString().split('T')[0],
          dayLabel: 'today',
          flavors: [{ name: "Check website", description: "Murf's flavor calendar available at website" }]
        }
      ]
    }];
    ...
    return [{
      ...
      flavors: [{ 
        date: new Date().toISOString().split('T')[0],
        dayLabel: 'today',
        flavors: [{ name: "Check website", description: "Visit murfsfrozencustard.com for today's flavor" }]
      }]
    }];
  }
}
  ...
  try {
    ...
    const flavorData = await page.evaluate(() => {
      ...
      dayHeaders.forEach(header => {
        ...
        if (headerText.includes('Today') || headerText.includes('Tomorrow') || 
            headerText.includes('Monday') || headerText.includes('Tuesday') ||
            headerText.includes('Wednesday') || headerText.includes('Thursday') ||
            headerText.includes('Friday') || headerText.includes('Saturday') ||
            headerText.includes('Sunday')) {
          
          // Find the flavor link/text nearby
          const parent = header.parentElement;
          const flavorLink = parent.querySelector('a[href*="flavor-of-the-day"]');
          
          if (flavorLink) {
            const flavorName = flavorLink.textContent.trim();
            const dayLabel = headerText.toLowerCase().includes('today') ? 'today' :
                            headerText.toLowerCase().includes('tomorrow') ? 'tomorrow' : '';
      ...
      flavors: flavorData.length > 0 ? flavorData : [
        {
          date: new Date().toISOString().split('T')[0],
          dayLabel: 'today',
          flavors: [{ name: "Check location", description: "Visit Culver's website for today's flavor" }]
        }
      ]
    };
    
  } catch (error) {
    console.error(`Error scraping Culvers ${locationData.slug}:`, error.message);
    await browser.close();
    return {
      ...locationData,
      id: `culvers-${locationData.slug}`,
      ...
      flavors: [
        {
          date: new Date().toISOString().split('T')[0],
          dayLabel: 'today',
          flavors: [{ name: "Check location", description: "Visit location for today's flavor" }]
        }
      ]
    };
  }
    ...
    flavors: [
      {
        ...
        dayLabel: 'always',
        flavors: [
          { name: "Vanilla", description: "Classic vanilla custard (always available)" },
          { name: "Chocolate", description: "Rich chocolate custard (always available)" },
          { name: "Butter Pecan", description: "Butter pecan custard (always available)" },
          { name: "Weekend Special", description: "Check in-store or Facebook for weekend specials" }
        ]
      }
    ]
  }];
...
async function scrapeGilles() {
  ...
  try {
        ...
        flavors: [
          {
            date: new Date().toISOString().split('T')[0],
            dayLabel: 'closed',
            flavors: [{ name: "Closed for Winter", description: "Returning January 8, 2026" }]
          }
        ]
      }];
    }
    ...
    const flavorHeading = $('.featured h3, h3:contains("Flavor")').first().text().trim();
    const flavorText = $('.featured p, p:contains("custard")').first().text().trim();
      ...
      flavors: [
        {
          ...
          flavors: [{
            ...
            description: flavorText || "Visit Gilles for today's special flavor"
          }]
        }
      ]
    }];
      ...
      flavors: [
        {
          date: new Date().toISOString().split('T')[0],
          dayLabel: 'today',
          flavors: [{ name: "Check website", description: "Visit website for current status" }]
        }
      ]
    }];
  }
...
async function scrapeAllStands() {
  ...
  try {
    ...
    console.log(`✅ Scraping complete! Found ${results.totalLocations} locations`);
...
cron.schedule('0 6 * * *', async () => {
  console.log('⏰ Running scheduled flavor scrape at', new Date().toLocaleString());
